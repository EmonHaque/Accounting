...
BEGIN
	WITH T AS(
	SELECT Id, [Date], D.CCODE AS DCCode, DSCode, C.CCODE AS CCCode, CSCode, DCPId, CCPId, PartyId, VcNo, TransRefId, LooseLotNo, 
			D3.InvCCode AS InvCCode, D3.InvSCode AS InvSCode,
			KGQtyRec, KGQtyIss, KGQtyRecAmt, KGQtyIssAmt
	FROM TTable
	...
	WHERE InvCCode = @InvCode AND (KGQtyRec IS NOT NULL OR KGQtyIss IS NOT NULL)
	)
	SELECT [Date], CASE 
	WHEN(D1.CCODE LIKE '201%' AND C1.CCODE LIKE '50207%') THEN C1.CHOA
	...
	WHEN(D1.CCODE LIKE '50204%' AND C1.CCODE LIKE '201%') THEN D1.CHOA	
	ELSE 'hmm?' END AS Particulars,
	VcNo, LooseLotNo AS LotNo, KGQtyRec AS QtyIn, KGQtyIss AS QtyOut, KGQtyRecAmt AS AmtIn, KGQtyIssAmt AS AmtOut, Inv1.CHOA AS MainCat, Inv2.SHOA AS SubCat, D3.CName AS Cat 
	INTO #Temp1 
	FROM T
